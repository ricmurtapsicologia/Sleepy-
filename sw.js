...
function patchHtml(html){
  if(!html.includes('css/refinements.css')){
    html=html.replace('</head>','  <link rel="stylesheet" href="css/refinements.css">\n</head>');
  }
...
async function htmlResponse(request){
  ...
  return new Response(patchHtml(text),{
...
self.addEventListener('fetch',event=>{
  ...
  if(event.request.mode==='navigate' || url.pathname.endsWith('/index.html') || url.pathname.endsWith('/')){
    event.respondWith(htmlResponse(event.request));
    return;
  }
